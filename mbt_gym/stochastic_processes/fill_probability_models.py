...
class CustomPowerFillFunction(FillProbabilityModel):
    def __init__(
        self,
        fill_exponent: float = 1.5,
        fill_multiplier: float = 1.5,
    ...
    def _get_fill_probabilities(self, depths: np.ndarray) -> np.ndarray:
        """
        Compute fill probabilities for bid and ask based on depth levels.
        """
        return_array = np.zeros(2)
        depth_pair = depths[0]

        for i, depth in enumerate(depth_pair):
            depth += 1  # Shift by 1 as per calibration

            if depth < 0:
                return_array[i] = 0  # Invalid negative depth
            elif depth < 5:
                return_array[i] = min(self.fill_multiplier * np.max(depth, 0) ** (-self.fill_exponent), 1)
            else:
                return_array[i] = 0.01  # Clipped fill chance
