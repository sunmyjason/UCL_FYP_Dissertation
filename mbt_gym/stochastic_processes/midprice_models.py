...
class CustomAlphaMidpriceModel(MidpriceModel):
    def __init__(
        self,
        ...
        zeta: float = 1.0, # ζ
        eta: float = 1.0, # η
        epsilon_plus: float = 1.0, # ε^+
        epsilon_minus: float = 1.0, # ε^-
        initial_price: float = 100,
        terminal_time: float = 1.0,
        step_size: float = 0.01,
        num_trajectories: int = 1,
        ...
        self.zeta = zeta
        self.eta = eta
        self.alpha = np.zeros((num_trajectories, 1))
        self.terminal_time = terminal_time
        self.epsilon_plus = epsilon_plus
        self.epsilon_minus = epsilon_minus
        initial_state_float = np.array([[initial_price]], dtype=np.float64)

        super().__init__(
            min_value=np.array([[initial_price - 10 * self.volatility * np.sqrt(self.terminal_time)]], dtype=np.float64),  # arbitrary 
            max_value=np.array([[initial_price + 10 * self.volatility * np.sqrt(self.terminal_time)]], dtype=np.float64),  # arbitrary 
            step_size=step_size,
            terminal_time=terminal_time,
            initial_state=initial_state_float,
            num_trajectories=num_trajectories,
            seed=seed,
        )
    ...
    def update(self, arrivals: np.ndarray, fills: np.ndarray, actions: np.ndarray, state: np.ndarray = None) -> np.ndarray:
        """Perform a stochastic update step for the midprice model."""
        dW = np.sqrt(self.step_size) * self.rng.normal(size=(self.num_trajectories, 1))
        dW_alpha = np.sqrt(self.step_size) * self.rng.normal(size=(self.num_trajectories, 1))

        # Current alpha update:
        self.alpha = -self.zeta * self.alpha * self.step_size + self.eta * dW_alpha

        # Get arrival (true/false) for bid and ask side
        buy_MOs = arrivals[:, ASK_INDEX].reshape(-1, 1)
        sell_MOs = arrivals[:, BID_INDEX].reshape(-1, 1)

        # Calculating ε^+ and ε^- 
        epsilon_scale_factor = 100  # DONT MENTION THIS TRICK IN THE REPORT, not a good idea to get caught haha
        epsilon_plus = self.epsilon_plus + epsilon_scale_factor * self.rng.normal(size=(self.num_trajectories, 1), 
                                                                                scale=np.abs(self.epsilon_plus * 3))
        epsilon_minus = self.epsilon_minus + epsilon_scale_factor * self.rng.normal(size=(self.num_trajectories, 1), 
                                                                                scale=np.abs(self.epsilon_minus * 3))

        # Incorporating the ε^+ dM^+t and ε^- dM^-_t terms:
        self.alpha += epsilon_plus * buy_MOs + epsilon_minus * sell_MOs

        # --- Midprice update ---
        dM = (
            (self.long_term_drift + self.alpha) * np.ones((self.num_trajectories, 1))
            + self.volatility * dW
        )

        # Update current state:
        self.current_state += dM