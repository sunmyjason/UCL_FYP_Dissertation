...
class AvellanedaStoikovAgent(Agent):
    # REMARK: see https://quant.stackexchange.com/questions/36400/avellaneda-stoikov-market-making-model
    def __init__(self, risk_aversion: float = 0.1, env: TradingEnvironment = None):
        ...
        # Use baseline_arrival_rate for Hawkes model
        if isinstance(self.env.model_dynamics.arrival_model, CustomArrivalModel):
            self.rate_of_arrival = self.env.model_dynamics.arrival_model.get_point_arrival_rate()
        else:
...
class CarteaJaimungalMmDiscreteAgent(Agent):
    def __init__(
        self,
        env: TradingEnvironment = None,
    ):
    ...
    def get_action(self, state: np.ndarray):
        if self.inventory_neutral:
            ...
            # round to integer (nearest tick) before returning
            return np.round(self._calculate_deltas(inventories=inventories, current_time=current_time))
