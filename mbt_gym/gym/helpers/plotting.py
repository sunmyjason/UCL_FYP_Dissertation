...
def plot_trajectory(env: gym.Env, agent: Agent, seed: int = None):
    ...
    ax3a.spines['right'].set_color('black')
    ax3a.spines['right'].set_linewidth(1)
    ax3a.tick_params(axis='y', colors='black')  # Make ticks match
    ...
    for i in range(env.num_trajectories):
        ...
        # Combine legends from both axes (inventory + cash holdings)
    ...
    # Remark on the data structure of observations, actions, and rewards:
    # It is always a 3D array, call that of shape (x, y, z).
    # x corresponds to the number of trajectories
    # y corresponds to the number of observations in each trajectory. For action, that is 2 
    # as it corresponds to bid-side spread and ask-side spread.
    # z corresponds to the number of timestamps in each observation.
    
    return asset_prices.flatten(), actions    
